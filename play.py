...
b2 = cv2.imread(scan_path + "calibration_B2.jpg")
b1 = cv2.imread(scan_path + "calibration_B1.jpg")
b0 = cv2.imread(scan_path + "calibration_B0.jpg")
h, w, _ = f2.shape

print('----- FRONT -----')
#corner = get_scalar(f0)
#f_corners = ((corner[0][0][0], corner[0][0][1]), (corner[5][0][0], corner[5][0][1]),
#             (corner[nx*nx][0][0], corner[nx*nx][0][1]), (corner[nx*nx + (nx-1)][0][0], corner[nx*nx + (nx-1)][0][1]))
# print(f_corners)
print('----- CENTER -----')
#get_scalar(c0)
print('----- BACK -----')
#corner = get_scalar(b0)
#b_corners = ((corner[0][0][0], corner[0][0][1]), (corner[5][0][0], corner[5][0][1]),
#             (corner[nx*nx][0][0], corner[nx*nx][0][1]), (corner[nx*nx + (nx-1)][0][0], corner[nx*nx + (nx-1)][0][1]))
# print(b_corners)

# line from f->b for each corner, find
new = np.zeros((h, w, 3), np.uint8)
new = cv2.subtract(f0, c0)
"""
